...
class PageController extends Controller
{
   ...
   {
       $dem = User::where('email',$request->email)->where('password',Hash::make($request->password))->count();
       $nguoidung = User::where('email',$request->email)->where('password',Hash::make($request->password))->first();
       if($dem != 0)
       {
           $request->session()->put('id_user',$nguoidung->id);
           $request->session()->put('name',$nguoidung->name);
