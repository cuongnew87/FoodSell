...
class PageController extends Controller
{
   ...
   public function postThanhToan(Request $request)
   {
       ...
       foreach($product_bill as $product_bill)
       {
           ...
           $bill->image = $product_bill->image;
           $bill->name = $product_bill->name;
           $bill->soluong = $product_bill->soluong;
           $bill->thanhtoan = $product_bill->thanhtoan;
           $bill->messenge = $request->messenge;
           $bill->method_payment = $request->method_payment;
           $bill->save();
       }
   ...
   public function postDangKy(Request $request)
   {
      ...
      [
        ...
        // 'password.regex'=>'Mật Khẩu của bạn không được bao gồm các kí tự đặc biệt'
      ]);
      $nguoidung = new User();
   ...
   {
       $dem = User::where('email',$request->email)->where('password',$request->password)->count();
       $nguoidung = User::where('email',$request->email)->where('password',$request->password)->first();
       if($dem != 0)
       {
           $request->session()->put('id_user',$nguoidung->id);
           $request->session()->put('name',$nguoidung->name);
