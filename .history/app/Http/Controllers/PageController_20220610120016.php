...
class PageController extends Controller
{
    ...
    public function postThanhToan(Request $request)
    {
        ...
        ->get();
        if($request->method_payment == "Thanh Toán Tiền Mặt"){
            ...
            $products = DB::select("select * from `products`,`bills` WHERE `bills`.`id_sanpham` = `products`.`id` AND `bills`.`id_user` = " . Session::get('id_user'));
            return view('page.LichSuMuaHang', ['bills' => $bills, 'nguoidung' => $nguoidung, 'products' => $products]);
        }else{
            $enableSandbox = true;
            $paypalConfig = [
                'email' => 'test@gmail.com',
                'return_url' => env('VARIABLE_NAME') . '/public/thanh-toan?paypal=success',
                'cancel_url' => env('VARIABLE_NAME') . '/public/thanh-toan?paypal=failed',
                'notify_url' => env('VARIABLE_NAME') . '/public/thanh-toan?paypal=notify',
            ];   
            ...
            $index = 1;
            foreach ($product_bill as $product_bill) {
                $itemAmount += ($product_bill->thanhtoan/23172.98);
                $data['item_name_' . $index] = $product_bill->id_sanpham;
                $data['amount_' . $index] = ($product_bill->thanhtoan/23172.98);
                $index ++;
            }
            $data['item_name'] = 'Món ăn';
    ...
    public function getGioHang()
    {
        $product_buy = DB::table('cart')
            ->where('cart.id_user', '=', Session::get('id_user'))
            ->join('products', 'cart.id_sanpham', '=', 'products.id')
            ->select('cart.id', 'cart.id_user', 'cart.soluong', 'cart.thanhtoan', 'products.name', 'products.image', 'products.unit_price', 'products.promotion_price', 'products.status')
            ->get();
        $total = DB::table('cart')
            ->where('cart.id_user', '=', Session::get('id_user'))
            ->join('products', 'cart.id_sanpham', '=', 'products.id')
    ...
    public function postUpdate_Review_cart(Request $request)
    {
        ...
        $update_cart = DB::table('cart')
            ->where('cart.id_user', '=', Session::get('id_user'))
            ->join('products', 'cart.id_sanpham', '=', 'products.id')
            ->select('cart.id', 'cart.soluong', 'cart.thanhtoan', 'products.name', 'products.image', 'products.unit_price', 'products.promotion_price', 'products.status')
            ->get();
        $total = DB::table('cart')
            ->where('cart.id_user', '=', Session::get('id_user'))
            ->join('products', 'cart.id_sanpham', '=', 'products.id')
    ...
    public function getChiTietSp(Request $request, $id)
    {
        ...
        $reviews = Review::where('id_sanpham', $id)->get();
        $product_seem = Product::where('id_type', $product_detail->id_type)->paginate(4);
        $request->session()->put('id_product_detail', $id);
        return view('page.ChiTietSp', ['product_detail' => $product_detail, 'product_seem' => $product_seem, 'reviews' => $reviews]);
    }
    ...
    public function postReview(Request $request, $id)
    {
        if (Session::has('id_user')) {
            ...
            $review = new Review();
            $review->id_sanpham = $id;
            $review->customer_name = $user->name;
            $review->customer_image = $user->avatar;
    ...
    public function getLichSuMuaHang($id)
    {
        ...
        return view('page.LichSuMuaHang', ['nguoidung' => $nguoidung, 'bills' => $bills, 'products' => $products]);
    }
