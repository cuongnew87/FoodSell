        ...
        <form action="cap-nhat-review-cart" method="post">
            ...
            <div class="table-responsive cart_info">
                <table class="table table-condensed">
                    ...
                    <tbody>
                        <?php $can_purchase = true; ?>
                        @foreach ($update_cart as $update )

                        <tr>
                            <td class="cart_product">
                                <a href=""><img src="source/image/product/{{$update->image}}" alt="" width="100px"></a>
                            </td>
                            <td class="cart_description">
                                <h4><a href="">{{$update->name}}</a></h4>
                            </td>
                            <td class="cart_price">
                                @if ($update->promotion_price != 0)
                                <span class="" id="promotion_price{{$update->id}}">{{$update->promotion_price}}.đ</span>
                                @else
                                <span class="" id="unit_price{{$update->id}}">{{$update->unit_price}}.đ</span>
                                @endif
                            </td>
                            <td>
                                @if($update->status == 0)
                                    <span>Hết hàng</span>
                                    <?php $can_purchase = false; ?>
                            ...
                            <td class="cart_quantity">
                                <div class="cart_quantity_button">
                                    <a class="cart_quantity_up"  onclick="tang({{$update->id}})"> + </a>
                                    <input type="hidden" name = "id[]" value="{{$update->id}}">
                                    <input class="cart_quantity_input" type="text" id ="{{$update->id}}" onchange="changeQuantity({{$update->id}})" name="soluong[]" value="{{$update->soluong}}" size="2" min="0">
                                    <a class="cart_quantity_down"  onclick="giam({{$update->id}})"> - </a>
                                </div>
                            </td>
                            <td class="cart_total">
                                <p class="cart_total_price" id="payment{{$update->id}}">{{$update->thanhtoan}}.đ</p>
                            </td>
                            <td class="cart_delete">
                                <a class="cart_quantity_delete" href="xoa-review-cart/{{$update->id}}"><i class="fa fa-times"></i></a>
                            </td>
                        </tr>
                        ...
                        <div class="col-sm-6">
                            <div class="total_area">
                                ...
                                <div class="total_area_button">
                                    ...
                                    @elseif(!$can_purchase)
                                        <a class="btn btn-danger" href="#" style="pointer-events: none; cursor: default;">Không thể thanh toán sản phẩm đã hết hàng</a>
                                    @endif
                                </div>
                            </div>
                        </div>
